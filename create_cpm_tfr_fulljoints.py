import cv2
import utils.cpm_utils as utils
import numpy as np
import math
...
for person_dir in os.listdir(dataset_dir):
    ...
    gt_content = codecs.open(gt_file, 'rb', encoding='utf-8').readlines()
    ...
    for idx, line in enumerate(gt_content):
        ...
        # for i in range(len(cur_body_joints_x)):
        #     cv2.circle(cur_img, center=(int(cur_body_joints_x[i]), int(cur_body_joints_y[i])),radius=3, color=(255,0,0), thickness=-1)
        #     cv2.imshow('', cur_img)
        #     cv2.waitKey(500)
        # cv2.imshow('', cur_img)
        # cv2.waitKey()
        ...
        if cur_img.shape[0] > cur_img.shape[1]:
            ...
            if SHOW_INFO:
                ...
                for i in range(num_of_joints):
                    cv2.circle(output_image, (int(cur_body_joints_x[i]), int(cur_body_joints_y[i])), 3, (0, 255, 0), 2)
            ...
            cur_body_joints_y = map(lambda x: x + (box_size / 2 - math.floor(image.shape[0] / 2)),
                                    cur_body_joints_y)
        ...
        coords_set = np.concatenate((np.reshape(cur_body_joints_x, (num_of_joints, 1)),
                                     np.reshape(cur_body_joints_y, (num_of_joints, 1))),
                                    axis=1)
